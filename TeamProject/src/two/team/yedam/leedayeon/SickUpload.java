...
public class SickUpload implements Commnad {
...
//텍스트 타입은 getoaraneter
			String desc = multi.getParameter("desc"); // form의 name="desc"인 값을 구함
			String de = multi.getParameter("");//input 값 담아올 것
			String des = multi.getParameter("");//이후에 DTO에 넘겨야 함
			...
			new FileInputStream( multi.getFile("uploadfile"));
			byte[] bt = FileUtil.fileToBinary(multi.getFile("uploadfile")) ;
			//dto.set(bt);
			//setBlob
			//dao insert하면서 dto 넣어주기
