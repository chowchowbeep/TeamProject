...
<script>
	...
	$(document).ready(function() {
		...
		var getListWithPaging = function(page, type) { //특정페이지 파라미터로 받아와서
		...
		var endPageBtn;
	  	...
	  	$.getJSON('ajax/SBeforeMedListPaging.do?id='+ id + '&'+'type'+ type, function(result) {
			//'ajax/SBeforeMedListPaging.do'로 제이슨 요청. 파라미터 주의. 
			
			//페이징을 위한 부분
			totalDataRows = result.totalDataRows; // 총게시물 갯수 설정
			totalPageCnt = Math.floor(totalDataRows / dataRowsPerPage);
			// 총 페이지수 구하기
	...
	function listAjax(type, firstIndex, lastIndex, currentPage) {
		...
		$.ajax({
			type : "POST",
			url : "ajax/SMedBeforeMedListAjax.do",
			dataType : "json",
			data : {
				"type" : type,
				"id" : id,
...
<script>
	$(window).on('popstate', function(event) { //state를 pop하게 될 경우- 뒤로가기
	  var data = event.originalEvent.state; // push한 state를 변수에 저장
	    $('tbody').html(data.tbody); // state객체를 특정 html에 붙임
	    $('#next').attr('href', '/pushstate/page' + (+data.page+1)); //다음 버튼  
	});
